...
;(function (platform) {
  /**
   * Show debug info if __DEBUG is set
   * @param  {Strin} value    [description]
   * @param  {Boolean=} suppress [description]
   */
  ...
  var messageContainer = document.getElementById(UNSUPPORTED_BROWSER_ID);
  ...
  /**
   * @param  {String} osString Operating system
   * @return {Number}
   */
  ...
  /**
   * Creates list of supported browsers for current system
   * @param  {String} os    
   * @param  {String} locale
   */
  ...
  /**
   * Info about current UNsupported browser
  ...
  function unsupportedBrowserInfo (browser, version) {
    ...
    if (toggler_.addEventListener) {
      toggler_ && toggler_.addEventListener('click', function(e) {
        toggleUnsupportedBrowserInfo(e)
      });
    } else {
      toggler_.attachEvent('onclick', function() {
        toggleUnsupportedBrowserInfo(toggler_);
      });
    }
  ...
  /**
   * Show or hide additional info about supported browsers
   * @param  {Event} e      
   */
  function toggleUnsupportedBrowserInfo (e) {
    if (!messageContainer) {
      return;
    }
    ...
    var info_ = document.getElementById(messageContainer.id + '--wrapper');
    ...
    $(document.body).trigger('resize.layout');
  }

  /**
   * Removes message if browser supported
   */
  ...
  /**
   * Moves message to main wrapper
   */
  function moveMessageToMainWrapper () {
   ...
   * @param  {Object=}  browsers Predefined rules for browser checking
   * @return {Object}            Browser and test result
   */
  window.isBrowserSupported = function (locale, browsers) {
    ...
    // When dom ready, move message
    document.addEventListener("DOMContentLoaded", moveMessageToMainWrapper);
